...
bytes!(Key, BLOCKSIZE);
bytes!(Nonce, IVSIZE);
both_bytes!(SBox, PrivateSBox, 256);
bytes!(RCon, 11);
...
fn sub_bytes(state: Block) -> Block {
    let mut st = state;
    for i in 0..16 {
        st[i] = SBOX[U8::declassify(state[i])].into();
    }
    st
}
...
fn sub_word(w: Word) -> Word {
    Word([
        SBOX[U8::declassify(w[0usize]) as usize].into(),
        SBOX[U8::declassify(w[1usize]) as usize].into(),
        SBOX[U8::declassify(w[2usize]) as usize].into(),
        SBOX[U8::declassify(w[3usize]) as usize].into(),
    ])
}
