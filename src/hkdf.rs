...
pub fn expand(prk: ByteSeq, info: ByteSeq, l: usize) -> ByteSeq {
    ...
    let mut t = ByteSeq::new_len(n as usize * PRK::capacity());
    for i in 0..n {
        let hmac_txt_in = if i == 0 {
            build_hmac_txt(ByteSeq::new(), info.clone(), U8(i+1))
        } else {
