...
fn enc_dec_test(m: Bytes, key: [u8; 32], iv: [u8; 12]) {
    let c = chacha(key, iv, m.clone()).unwrap();
    let m_dec = chacha(key, iv, c).unwrap();
    assert_eq!(m, m_dec);
}

fn kat_test(m: Bytes, key: [u8; 32], iv: [u8; 12], exp_cipher: Bytes, valid: bool) {
...
fn test_kat() {
    ...
    let exp_cipher = Bytes::from_vec(vec![
